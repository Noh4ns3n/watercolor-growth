    ...
    const render = () => {
      ...
      if (simCanvas && imgLoaded && simCanvas.width > 0 && oCtx) {
        ...
        if (frameCounter % 10 === 0 && audioStarted) {
          ...
          const realityData = oCtx.getImageData(0, 0, analysisSize, analysisSize).data;

          // Draw sim to offscreen
          oCtx.drawImage(simCanvas, 0, 0, analysisSize, analysisSize);
          const simData = oCtx.getImageData(0, 0, analysisSize, analysisSize).data;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      {!audioStarted && (
        <button 
          onClick={initAudio}
          style={{ position: 'absolute', top: 10, left: 10, zIndex: 10, padding: '10px' }}
        >
          Initialize Audio
        </button>
      )}
