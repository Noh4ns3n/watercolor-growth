...
export default function SimulationPanel({ latestImageSrc, canvasRef }: Props) {
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Calculate the actual rendered dimensions within the bounding box
    ...
    // Map the click accurately to the internal 500x500 resolution
    const x =
      ((e.clientX - rect.left - offsetX) / renderedWidth) * canvas.width;
    const y =
      ((e.clientY - rect.top - offsetY) / renderedHeight) * canvas.height;
    ...
    img.onload = () => {
      ...
      if (brightPixels.length > 0) {
        for (let i = 0; i < INJECTION_COUNT; i++) {
          const randomPixel =
            brightPixels[Math.floor(Math.random() * brightPixels.length)];
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // 1. Draw Attractants (Food) onto Trail Map as a chemical gradient BEFORE diffusion
      attractantsRef.current.forEach((oat) => {
        ...
        gradient.addColorStop(0, "rgba(0, 255, 0, 1)"); // Pure Green
        gradient.addColorStop(1, "rgba(0, 255, 0, 0)");

        offCtx.fillStyle = gradient;
        offCtx.beginPath();
        offCtx.arc(oat.x, oat.y, 40, 0, Math.PI * 2);
        offCtx.fill();
      });

      // 2. DIFFUSION on Trail Map
      ...
      const getIntensity = (x: number, y: number) => {
        ...
        const index = (Math.floor(y) * CANVAS_SIZE + Math.floor(x)) * 4;
        // Return Red (trails) + Green (food scent)
      ...
      offCtx.fillStyle = "rgba(255, 0, 0, 0.4)"; // Pure Red

      agentsRef.current.forEach((agent) => {
        const fVal = getIntensity(
          agent.x + Math.cos(agent.angle) * sd,
          agent.y + Math.sin(agent.angle) * sd,
        );
        const lVal = getIntensity(
          agent.x + Math.cos(agent.angle - sa) * sd,
          agent.y + Math.sin(agent.angle - sa) * sd,
        );
        const rVal = getIntensity(
          agent.x + Math.cos(agent.angle + sa) * sd,
          agent.y + Math.sin(agent.angle + sa) * sd,
        );
        ...
        offCtx.beginPath();
        offCtx.arc(agent.x, agent.y, 0.6, 0, Math.PI * 2);
        offCtx.fill();
      });
      ...
      // 7. Draw the visible Oat Flakes on top of everything
      displayCtx.fillStyle = SIMULATION_COLORS.food;
      attractantsRef.current.forEach((oat) => {
        displayCtx.beginPath();
        displayCtx.arc(oat.x, oat.y, 3, 0, Math.PI * 2);
        displayCtx.fill();
      });
      ...
      <div className="sim-params-menu">
        ...
        {menuOpen && (
          <div className="sim-params-content">
            <label>
              Sensor Distance: {uiParams.sensorDistance}
              <br />
              <input
                type="range"
                min="3"
                max="30"
                value={uiParams.sensorDistance}
                onChange={(e) =>
                  handleParamChange(
                    "sensorDistance",
            ...
            <label>
              Sensor Angle (°): {uiParams.sensorAngle}
              <br />
              <input
                type="range"
                min="10"
                max="90"
                value={uiParams.sensorAngle}
                onChange={(e) =>
                  handleParamChange("sensorAngle", parseFloat(e.target.value))
                }
              />
            </label>
            <label>
              Growth Speed: {uiParams.stepSize.toFixed(1)}
              <br />
              <input
                type="range"
                min="0.05"
            ...
            <label>
              Evaporation: {uiParams.decay.toFixed(3)}
              <br />
              <input
                type="range"
                min="0.01"
            ...
            <hr
              style={{ borderColor: "var(--border-color)", margin: "10px 0" }}
            />
            <button
              onClick={() => (attractantsRef.current = [])}
              className="sim-params-clear"
            >
              Clear Attractants
            </button>
          </div>
